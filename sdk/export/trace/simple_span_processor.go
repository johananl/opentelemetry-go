...
func (ssp *SimpleSpanProcessor) OnStart(parent context.Context, s trace.ReadWriteSpan) {
}
...
func (ssp *SimpleSpanProcessor) OnEnd(s trace.ReadOnlySpan) {
	if ssp.e != nil && s.SpanContext().IsSampled() {
		if err := ssp.e.ExportSpans(context.Background(), []trace.ReadOnlySpan{s}); err != nil {
			otel.Handle(err)
		}
	}
}
